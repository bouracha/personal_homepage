...
//OPTIMISATION: removed array initialisations as these were dynamic pointer arrays
 ...
 */
  ...
  for(j=1;j<=Nsave;j++){
      start=second();// start timing
      evolve(Nstep,dt); 
      stop=second();// end timing
